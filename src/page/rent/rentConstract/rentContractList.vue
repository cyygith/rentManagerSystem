...
export default {
    ...
    methods:{
        ...
        //编辑详情
        toDetail(item){
            this.$router.push({path:'rentContractDetail',query:{id:item.id}});
        },
        // 获取详情
        queryList(){
        ...
        //修改
        detail(){
            this.$router.push({path:'rentContract',query:{id:this.chooseItem.id}});
        },
        //新增
        add(){
            this.$router.push({path:'rentContractModify',query:{ifNew:true}});
        },
        //删除
        del() {
            this.$confirm('确定删除该记录?', '提示', {confirmButtonText: '确定',cancelButtonText: '取消',type: 'warning'}).then(() => {
                ...
                contractApi.delete(param).then(res => {
                    if (res.code == "0") {
                        this.$message({
                            type: 'success',
                            message: '删除成功!',
